...
def change_led():
    sio.write((f"{'s' if request.json['selected'] else 'c'}\
{request.json['shelf_id']:0{8}X}{request.json['row']:0{2}X}\
{request.json['col']:0{2}X}").encode())
    print(f"led shelf: {request.json['shelf_id']} \
row: {request.json['row']} \
col: {request.json['col']} \
;turned {'on' if request.json['selected'] else 'off'}")
    return ('',200)
